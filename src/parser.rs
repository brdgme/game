    ...
    I: Iterator<Item = &'a (S, T)>,
{
    ...
    match matching.len() {
        1 => Ok(&matching[0].1),
        0 => Err(
            ErrorKind::InvalidInput("Couldn't find any matching options".to_string()).into(),
        ),
...
mod test {
    ...
    fn non_spaces_works() {
        assert_eq!(
            parser(non_spaces).parse("egg bacon cheese"),
            Ok(("egg".to_string(), " bacon cheese"))
        );
        assert_eq!(
            parser(non_spaces).parse("egg\nbacon cheese"),
            Ok(("egg".to_string(), "\nbacon cheese"))
        );
    }
    ...
    fn quoted_single_works() {
        assert_eq!(
            parser(quoted_single).parse("'egg bacon 'cheese"),
    ...
    fn quoted_double_works() {
        assert_eq!(
            parser(quoted_double).parse("\"egg bacon \"cheese"),
            Ok(("egg bacon ".to_string(), "cheese"))
        );
    }
